...
Use this skill when the user asks to run the source command `standup`.
...
2. Read the entries in `wiki/log.md` and query git commits (using `git log`) within that specified time window. Group findings by project.
