---
name: "source-command-pull-sources"
description: "Pull fresh material from real-world sources into mirror/pulled/ for triage"
---

# source-command-pull-sources

Use this skill when the user asks to run the migrated source command `pull-sources`.

## Command Template

You are in run-command mode to pull fresh material into mirror/pulled/ so the user can
triage it, then run /ingest. Source 1 (Codex sessions) is always included; other
...
## Source 1 — Codex sessions (always, window 7 days)
- Read JSONL from `C:/Users/THINKPAD L13/.Codex/projects/**/*.jsonl`, keep sessions whose
  first message is within the last 7 days, all subdirectories.
- One file per session → `mirror/pulled/Codex-sessions/<date>-<slug>.md`.
...
- Frontmatter: `source: Codex`, `captured`, `id` (= session id), `session_id`, `tags` (matched project).
...
1. Append ONE timestamped `wiki/log.md` entry: date + per-source counts, e.g.
   `2026-08-04 - pulled: Codex 3, gmail 12, calendar 0, local 5, github (skipped), notion (skipped)`.
