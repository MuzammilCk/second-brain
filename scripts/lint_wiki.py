...
if (VAULT_ROOT / "core" / "wiki").exists():
    WIKI_DIR = VAULT_ROOT / "core" / "wiki"
elif (VAULT_ROOT / "wiki").exists():
    WIKI_DIR = VAULT_ROOT / "wiki"
elif (Path(__file__).resolve().parent / "core" / "wiki").exists():
    WIKI_DIR = Path(__file__).resolve().parent / "core" / "wiki"
else:
...
def lint_wiki() -> int:
    ...
    print("\n­ƒöì Running Codex Vault Wiki Linter...")
    ...
    print(f"­ƒôæ Indexed {len(all_files)} markdown files & slug keys in wiki.")
    ...
    if errors:
        print(f"ÔØî {len(errors)} Error(s) Found:")
        for err in errors:
            print(f"   {err}")
        print("=" * 60)
        print("ÔØî Linter failed. Please resolve the errors above.")
        return 1

    print(f"Ô£à SUCCESS: All {len(all_files) // 2} wiki notes are valid!")
